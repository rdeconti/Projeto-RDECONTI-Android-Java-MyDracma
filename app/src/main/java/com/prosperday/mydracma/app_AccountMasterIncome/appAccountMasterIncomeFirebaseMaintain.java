...
public class appAccountMasterIncomeFirebaseMaintain {
    ...
    public static boolean appAccountMasterFirebaseUpdate(
        ...
        try {
            ...
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterTransaction").setValue(strTransaction);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterCategory").setValue(strCategory);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterSubCategory").setValue(strSubCategory);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterName").setValue(strName);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterStartDate").setValue(strStartDate);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterFinishDate").setValue(strFinishDate);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterExpireDate").setValue(strExpireDate);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterInitialValue").setValue(strInitialValue);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterLimitValue").setValue(strLimitValue);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterTaxes").setValue(strTaxes);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterInterest").setValue(strInterest);

            new appSupportCurrentDateTime();
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterUpdatedBy").setValue(myUserCode);
            firebaseAccountMaster.child(appAccountMasterIncomeReportAdapter.chooseFromUser).child("AccountMasterUpdatedOn").setValue(myDateTime);
